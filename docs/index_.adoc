...
参考情報: link:https://hono.dev/docs/getting-started/basic[Hono / Getting Started]
...
`myAPIserver`というディレクトリが作られる。その中にcdして、必要なライブラリをインストールしよう。
...
`src/index.ts` を開いてみよう。下記のコードが書かれているはずだ。
...
下記のコマンドを実行するとサーバーが立ち上がるはずだ。 http://127.0.0.1:3000/ をブラウザで開けばこんな画面が見られるはずだ。

image::https://kazurayam.github.io/KzHonoProjectBase/images/myAPIserver_1_index_initial.png[]

Bunの上で最も簡素なHonoのサーバーを立ち上げることができた。

次に `src/index.ts` を編集してAPIサーバのコードに改変していく。
